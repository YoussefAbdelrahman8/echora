...
class OCRReader:
    ...
    def load_model(self):
        ...
        logger.info(
            ...
            f"gpu={use_gpu}  english={'yes' if self._ocr_en else 'no'}"
        )
    ...
    def _run_ocr_cached(self, frame: np.ndarray) -> List[Dict]:
        ...
        otherwise acquires the inference lock and runs a fresh PaddleOCR pass.
        Double-checked locking pattern prevents redundant inference when two
        threads race to refresh the cache simultaneously.
        """
    ...
    def _preprocess(self, frame: np.ndarray) -> np.ndarray:
        """
        Brightness-adaptive contrast enhancement on the full-color BGR frame.
        PaddleOCR handles its own grayscale conversion internally, so color is
        preserved and luminance is adjusted via the LAB L-channel only.
        Sharpening is applied last to help character edge detection.
        """
        try:
            ...
            if mean_brightness < 100:
                clip = 4.0 if mean_brightness < 60 else 2.5
